...
const UpdateFamilleService = () => {
  ...
  const [image, setImage] = useState(null); // Nouvelle image sélectionnée
  const [existingImage, setExistingImage] = useState(null); // Image existante de l'API
  ...
  useEffect(() => {
    const fetchFamilleService = async () => {
      try {
        ...
        setExistingImage(data.imageUrl); // Récupère l'URL de l'image existante
      } catch (error) {
        console.error(error);
  ...
  const handleSubmit = async (e) => {
    ...
    if (image) {
      formData.append('imageUrl', image); // Ajoute la nouvelle image si elle existe
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Échec de la mise à jour : ${data.message || 'Erreur inconnue'}`, {
          position: "top-right",
          autoClose: 3000,
        });
      }
  ...
  const removeImage = () => {
    setImage(null); // Supprime la nouvelle image sélectionnée
    setExistingImage(null); // Supprime l'image existante du state local
  };
  ...
  return (
    <ModalWrapper>
      ...
      >
        ...
        >
          Mettre à jour Famille de Services
        </Title>
        <Form onSubmit={handleSubmit}>
          ...
          <DropZone
            ...
            custom={2}
          >
            <p>Glissez une nouvelle image ou</p>
            <UploadButton
              type="button"
